...
predictable and efficient memory footprint and sufficient computation speed for research applications on large (100s of millions -
1 billion parameters) models of various architectures using consumer-grade hardware.
...
As layers in deep learning architectures begin to evolve to more and more complex modules which compound many operations, we begin to see
issues in performing backpropagation methods at the per-module level of abstraction, the current widely-agreed upon choice for LRP.
...
While many modules are certainly defined by a sole operation such as Linear, Convolution, Softmax, etc., there is a rising number of modules
that compound these with elementary tensor operations, which are not captured at the granularity of modules.

The first consequence is that there is no upper limit for the number of unique module types, and as the field advances, the rate at which
these new modules are introduced increases, each requiring a new backpropagation rule. This is unsustainable in the long-term.
By shifting to the level of operations, there is a strict set of operation-specific rules that must be defined before all architectures
that reduce to that set of operations are supported.

Another complication of elementary operations (e.g. addition, multiplication, logarithm) not being captured at the module-level granularity
is that they must always appear only as sub-steps within a wider-scoped, module-specific propagation rule if they are to be considered in LRP.
Thus, redundancy arises when multiple modules make use of the same elementary operations, and although the logic for handling them may already
be defined for one module, it must be redefined for the others due to the propagation boundaries lying at the extremities of the modules and
not the operations. This puts limits on the usefulness and wide adoption of the backpropagation method.
...
More specific to current LRP implementations \textcolor{red}{Mention and cite?}, many modules in a model need to be patched and hooked so
that the built-in backpropagation machinery can be reused. While this method may be more performant at times, it requires a configuration
tailored specifically for each model, posing the same issue as the limitless number of module rule definitions. This process also explicitly
changes the behaviours of the forward and backward passes, and introduces a potential entrypoint for user error, should they use the wrong
version of the model, or forget to switch off these patches for true training or evaluation. A less pertinent detail, though possibly more so
for future applications, is that this also takes away the ability to operate on the model in both the traditional fashion and for
LRP simultaneously, since the computation graph can only be in one state at a time, and is required to remain as such for the whole duration
of the respective function.

By atomicizing relevance propagation to the individual operations applied to tensors, and relying solely on the computation graph, while not
interfering with its true structure, we eliminate the need for redundant model configurations, maximize reuse of already defined
...
repeatable fashion for any arbitrary model architecture. \textcolor{red}{This can probably also fit well in the Introduction.}

Our operation-level approach modularizes relevance propagation, reducing any model architecture to a fixed set of fundamental
...
\begin{definition}[Promise Branch Arg Node]
  A Promise Branch's Arg Node refers to the first Node along a Promise Branch in which its forward pass output is retrievable from its own cached tensors.\\
\end{definition}
...
We provide a full example of this mechanism in the Appendix.
...
The introduction of Promises can cause a circular dependency between Promise Branches. Consider the following configuration of nodes. Let A be a Promise-Generating Node, and let B and E be Arg Nodes, coloured orange.
...
While we are optimistic that a generalized form of LRP will empower and enable researchers to explain increasingly complex model architectures, we are equally aware that by breaking LRP down to the level of tensor operations,
we knowingly take on the task of covering a sufficiently large portion of the operation space to make this useful across a wide range of models.
We make a few considerations to address such concerns.

Firstly, recall that the set of tensor operations we are interested in, specifically mathematical and shape operations, is a bounded and well-defined set. 
This places a tangible and conceptual limit on our implementation scope. We also argue that defining rules for operations is more trivial than defining rules for modules, since one needs only to understand the operation and its
mathematical properties in isolation, and not additionally the context in which the operation is defined in. This better reflects the properties of the Chain Rule.

Another consideration is that there are many operation types whose propagation rules are easily defined.
Shape operations do not numerically transform any parts of a tensor, only the structure. Thus, the task of backpropagating relevance through such operations reduces to the task of reassigning or accumulating relevance at the
input positions that produced the output. In these cases, the desired functions for LRP are identical to the ones used for gradient backpropagation, and are accessible to us via the auto-differentiation library.
In addition, previous LRP literature has established that activation functions such as ReLU, GELU, etc. correspond to stable propagation rules which simply pass relevance along unchanged, known as Identity Rules.
Fused operations and renamed combinations of existing primitives can be either directly routed to already-supported operators, or be decomposed during auxiliary graph construction (Algorithm \ref{alg:graph-construction}) into
...
we claim that the Promise-based Dynamic LRP can be extended to any deep learning framework with a graph-based auto-differentiation library.
