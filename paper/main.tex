...
One of the more recent and impactful architectural advancements in LRP theory was AttnLRP, which derived novel propagation rules
for the unique components in attention blocks through Deep Taylor Decomposition \cite{achtibat2024attnlrp}.
AttnLRP showed clear empirical improvements over existing explainability methods (SmoothGrad, GradCAM, KernelSHAP, AttnRollout,
...
with desired human-friendly behaviours \cite{hatefi2025pruningcircuitcompression}.

Previous to this, uses of LRP have been widely explored for bias mitigation in image captioning and classification tasks. Anders et al.
proposed additional learned layers which consume LRP pixel heatmaps to identify and unlearn the associations to out-of-distribution
artifacts while amplifying those of the true object information. This targeted behaviour known as the Clever Hans effect, and introduced a
novel training framework leveraging spectral clustering to cleanse training data \cite{anders2020cleverhans}.
...
arbitrary DNNs with ISNet \cite{bassi2024isnet}, \cite{bassi2024fasterisnet}, and remained applicable for Attention-LSTM hybrids such
...
In addition, DynamicLRP clearly outperforms the Zennit \cite{anders2021zennit} implementation in the VGG evaluations as well.
...
We show that these two methods are mathematically equivalent. \textcolor{red}{Show a derivation in the Appendix}
