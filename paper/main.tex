...
processes of large-scale architectures e.g. transformer-based foundation models \cite{radford2019language, dosovitskiy2020image, touvron2023llama} remain challenging to interpret comprehensively.
...
Among these, Zennit \cite{anders2021zennit}  stands out as a comprehensive explainability library for PyTorch, 
focusing on LRP and other rule-based methods \cite{anders2021zennit}. Zennit remains one of the most flexible, 
readily available, and up-to-date libraries for LRP, providing a strong foundation for our work.
 Its implementation of LRP, when compared to other libraries such as \texttt{Captum} \cite{kokhlikyan2020captum} and \texttt{iNNvestigate} \cite{alber2018innvestigate}, demonstrates a clear emphasis on modularity and extensibility.
Anders et al. base their framework on the PyTorch \texttt{Module} structure, making great use of \texttt{Module Hooks} to transform the
...
\texttt{AttnLRP}, a recent advancement which derived new propagation rules suitable for attention modules, provides an implementation
...
\subsection{Computation Graphs and Automatic Differentiation}

Deep learning frameworks represent neural networks as computation graphs, formally defined as a directed acyclic graph (DAG) $G = ({V}, {E})$. Here, nodes $v \in {V}$ represent primitives (e.g., matrix multiplication, convolution, summation), and edges $(u, v) \in {E}$ indicate that the output of operation $u$ is an input to operation $v$.

Automatic differentiation (AD) engines operate on this graph to compute gradients. For each operation node $v$, the engine retains a local backward function that computes the Vector-Jacobian Product (VJP). During backpropagation, the engine traverses the graph in reverse topological order, passing gradient signals from the network output back to the inputs and parameters.

DynamicLRP exploits this architecture by introducing a parallel mechanism for relevance propagation. We map each operation node $v$ in the computation graph to a specialized relevance rule $\mathcal{R}_v$. Just as AD propagates gradients, our framework propagates relevance scores $R$ in the reverse direction of the forward pass. This operation-centric approach naturally decomposes complex layers into their constituent primitives. Consequently, the conservation property of LRP is strictly enforced at the primitive level:
\begin{equation}
\sum_{i \in \text{inputs}(v)} R_i = \sum_{j \in \text{outputs}(v)} R_j
\end{equation}
This ensures that the total relevance is preserved across the entire graph, regardless of the network's architectural complexity.
