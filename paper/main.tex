...
The rapid advancement of deep learning has revolutionized numerous domains, yet the increasing complexity of high-performing models has raised significant concerns about their interpretability and trustworthiness. Modern transformer architectures \cite{vaswani2017attention}, while achieving impressive performance, present unique challenges for understanding their decision-making processes. Attention maps, though useful for visualizing token interactions \cite{clark2019does, caron2021emerging}, fail to provide holistic explanations \cite{wiegreffe2019attention}, as critical information is often stored in feed-forward neurons separate from attention layers \cite{geva2021transformer, dai2022knowledge}.

Existing attribution methods fall into three main categories: gradient-based approaches (e.g., Input $\times$ Gradient \cite{simonyan2014deep}), which suffer from noisy gradients; perturbation-based methods (e.g., SHAP \cite{lundberg2017unified}), which are computationally expensive; and rule-based techniques like Layer-wise Relevance Propagation (LRP) \cite{bach2015pixel}, which offer customizable propagation rules within the Deep Taylor Decomposition framework \cite{montavon2017explaining}. Despite LRP's theoretical advantages, applying it to transformers has been challenging due to operations like softmax and layer normalization \cite{ding2017saliency, voita2021analyzing}.

Recent work has addressed some of these technical challenges. Achtibat et al. \cite{achtibat2024attnlrp} developed AttnLRP, demonstrating that LRP can faithfully explain transformer models by deriving novel propagation rules for attention mechanisms. However, LRP methods continue to be systematically overlooked by the machine learning community. A striking example is Machiraju et al.'s \cite{machiraju2024prospector} "Prospector Heads" work, which comprehensively evaluated gradients, attention maps, SHAP, and LIME for feature attribution, but entirely omitted LRP methods—despite their proven effectiveness on transformer architectures. This reflects a broader trend where LRP's potential remains underexplored, particularly for modern architectures that could benefit from its rule-based flexibility and computational efficiency.

To address these limitations and demonstrate LRP's untapped potential, we propose a model-agnostic attribution algorithm that extends LRP's operation-level granularity to any neural network architecture. Our approach leverages a promise-based system to handle missing forward activations, enabling faithful relevance propagation through complex computation graphs with computational efficiency comparable to standard backpropagation. By decomposing LRP rules to individual tensor operations—analogous to how PyTorch's autograd generalized gradient computation—our method achieves broad compatibility with modern architectures while maintaining high attribution quality, positioning LRP as a competitive alternative to currently dominant attribution paradigms.

\section{Background}

Modern transformer architectures, powered by the attention mechanism \cite{vaswani2017attention}, have revolutionized machine learning by enabling models to handle multimodal data and scale to massive datasets. However, their complexity introduces challenges in interpretability, particularly in understanding latent reasoning processes that span both attention layers and feed-forward networks, where factual knowledge is often stored \cite{geva2021transformer, dai2022knowledge}.
