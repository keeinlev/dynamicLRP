...
Additionally, Promises are propagated through the graph as relevance inputs themselves. A crucial consequence of these invariants is that the internal nodes recorded
within each Promise's computation path are mutually exclusive among all Promises, though multple Promises may share the same Origin or Arg nodes.
Together, these properties allow us to define clear upper bounds on the computational and memory overhead introduced by the Promise system.
...
Let $P \subseteq V$ be the set of promise-generating operations in the computation graph $G = (V, E)$. A promise-generating operation is an autograd node $p$ where exactly one of the following is true:
\begin{itemize}
  ...
  \item \textbf{OR} $p$ represents a function which is not a bijection, meaning the flow of information across it is not one-to-one in terms of its inputs and outputs.
\end{itemize}
...
During the initial pass, propagation of Promises may cause the traversal to temporarily disobey this ordering for the sake of searching ahead for missing activations.
