...
def get_model_and_tokenizer():
    ...
    # T5's embedding layer
    return model, tokenizer, model.shared
...
def run_captum_attribution(method, model, inputs_embeds, attention_mask, start_ind, end_ind):
    ...
    if method == "ig":
        ...
        # using internal_batch_size to save memory
        ...
        # Manual batching for GradientShap to avoid OOM
        n_samples = 50
        batch_size = 5
        total_attr = None
        remaining = n_samples
        ...
        while remaining > 0:
            ...
            # stdevs=0.0 means we just sample from baseline-input line (like IG but randomized)
            # or usually we add noise. Captum defaults stdevs=0.0 for baselines reference.
        ...
        # Feature mask: treat each token as a feature, not each float
        # Shape: [1, seq, dim]. mask[0, i, :] = i
        seq_len = inputs_embeds.shape[1]
        feature_mask = torch.arange(seq_len, device=device).unsqueeze(0).unsqueeze(-1).expand_as(inputs_embeds_float)
        
        # KernelShap is expensive. restrict n_samples.
        attributions = ks.attribute(inputs_embeds_float, baselines=baseline, feature_mask=feature_mask, n_samples=100)
    
    else:
        raise ValueError(f"Unknown method: {method}")

    # Reduce over embedding dimension to get token relevance
    return attributions.sum(dim=-1).detach().cpu()


def run_t5_squad_benchmark(model, tokenizer, embedding_layer, dataset, run_name, method, question_first=True, model_topk=2):
    results = []
    top1_label_hits = 0
    top1_model_hits = 0
    total_examples = 0
    ...
    for example in tqdm(dataset["validation"]):
        ...
        found_start = False
        found_end = False
        lrp_topk_inds = param_vals_2.flatten().topk(model_topk).indices
        ...
        if model_exact_match:
            # Captum relevance can be negative (unlike Gamma LRP), so we check > 0
            # Normalizing to match the logic roughly, though 2*norm -1 is specific to the values range
            # For general attribution, we just check positive influence? 
            # The original code assumed values in a range. Here we check positive attribution.
            # pos_inds = torch.where(param_vals_2.flatten() > 0)[0] 
            
            # Replicating exact logic of valid indices:
            # Original: torch.where((2 * param_vals[2].flatten() / param_vals[2].flatten().max() - 1).cpu() > 0)[0]
            # This selects values effectively in the upper half of the range [0, Max] if Min is 0
            ...
            strict_label_union = int(end - start + 1)
            ...
            # Handle potential division by zero if empty sets
        ...
        if not (total_examples % 100):
            # Printing periodic stats to stdout
    ...
    try:
        ...
        print(f"Results saved to t5_squadv2_results_{run_name}.json")
    except TypeError:
        print("Failed JSON write, attempting pickle...")
        try:
            with open(f"t5_squadv2_results_{run_name}.pkl", "wb") as f:
                pickle.dump(data, f)
            print(f"Results saved to t5_squadv2_results_{run_name}.pkl")
...
if __name__ == "__main__":
    ...
    # Optional subset for debugging/fast run
    target_dataset = dataset
    if args.num_samples:
        target_dataset = {"validation": dataset["validation"].select(range(args.num_samples))}

    run_name = f"captum_{args.method}"
    run_t5_squad_benchmark(model, tokenizer, embedding_layer, target_dataset, run_name, args.method)
