...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \State \textbf{Input:} Model output $hidden\_states$
    ...
    \State \Call{DFS}{$root$, $in\_adj$, $out\_adj$, $visited$, $topo\_stack$} \Comment{Start traversal}
    \State \Return $in\_adj$, $out\_adj$, $topo\_stack$ \Comment{In-adjacency list, Out-adjacency list, Topologically sorted nodes}
    \Function{DFS}{$fcn$, $in\_adj$, $out\_adj$, $visited$, $topo\_stack$}
      \If{$fcn = \text{None}$ or $fcn$ in $visited$} \Comment{Skip processed}
        \State \Return
      \EndIf
      \State $visited \gets visited \cup \{fcn\}$ \Comment{Mark visited}
      \For{each $child$ in $fcn.next\_functions$}
        \State $out\_adj[fcn].append(child)$ \Comment{Record dependencies}
        \State $in\_adj[child].append(fcn)$
        \State \Call{DFS}{$child$, $in\_adj$, $out\_adj$, $visited$, $topo\_stack$} \Comment{Recurse}
      \EndFor
      \State $topo\_stack.push(fcn)$ \Comment{Add to sort}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \State \textbf{Input:} Model output $hidden\_states$, Relevance interpretation target $target\_node$
    \State Initialize topologically sorted nodes, $stack$ auxiliary graph $in\_adj$, $out\_adj$, Output relevance $R_{out}$ according to traversal order from Algorithm \ref{alg:graph-construction}
    \Repeat
      \State $curnode \gets stack.pop()$ \Comment{Process next node}
      \State $curnode\_in\_rel = node\_inputs[curnode]$
      \If{$curnode = target\_node$} \Comment{Target reached}
      ...
      \ElsIf{$curnode$ requires Promise}
        ...
        \State $curnode\_outputs \gets curnode.promise.branches$ \Comment{Defer computation}
      \Else
        \State $prop\_fcn \gets fcn\_map[type(curnode)]$
        \State $curnode\_outputs \gets prop\_fcn(curnode\_in\_rel)$ \Comment{Apply LRP rule}
      \EndIf
      \For{each $child, output$ in $zip(out\_adj[curnode], curnode\_outputs)$}
        \State $node\_inputs[child].add(output)$ \Comment{Accumulate inputs}
        \State $nodes\_pending[child] \gets nodes\_pending[child] - 1$ \Comment{Update dependencies}
        \If{$nodes\_pending[child] = 0$}
          \State $stack.push(child)$ \Comment{Enqueue ready child}
        \EndIf
      \EndFor
    \Until{$stack$ is empty} \Comment{All nodes processed}
    \State \Return Input relevance $R_{in}$
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \State \textbf{Input:} Autograd Node $node$, Propagation input $R_{out}$
    \State \textbf{Output:} Propagation output list $R_{in}$
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \If{$R_{out}$ is a Promise Branch} \Comment{Retrieve forward activation}
      ...
      \If{$R_{out}.promise\_is\_complete$} \Comment{Resume propagation?}
