...
\iftoggle{arxiv}{
  ...
  \fancyhead[R]{Lee \textit{et. al.,} (2025)}
...
\author[1]{Kevin Lee}
\author[2, \textdagger]{Duncan Smith-Halverson}
\author[2, \textdagger]{Pablo Millan Arias}
\affil[1]{University of Waterloo}
\affil[2]{Scotiabank AI Resea—rch}
\affil[$\text{\textdagger}$]{Senior Author}
\affil[ ]{%
%\texttt{k327lee@uwaterloo.ca}
}
...
\begin{abstract}
Understanding why neural networks make specific predictions is essential for building trust in artificial intelligence systems, particularly in high-stakes domains. 
Among the many interpretability algorithms, Layer-wise Relevance Propagation (LRP) stands out as one of the foundational methods in neural network interpretability, 
offering a principled approach to attribution by redistributing prediction scores back to input features while preserving conservation properties. 
Although existing LRP implementations support classic architectures, they require a customised implementation for each new network architecture, which limits
their applicability to rapidly evolving model designs and preventing their widespread adoption in the interpretability research 
community. To address this limitation, we introduce DynamicLRP, a model-agnostic LRP framework that operates at the primitive level and decomposes 
LRP attribution rules to individual tensor operations within the network's computation graph. This is achieved through our novel mechanism for 
deferred activation resolution, named the Promise System, which maintains LRP's theoretical guarantees while achieving
true generality across model architectures. Our approach operates alongside standard backpropagation machinery without interference,
making it readily deployable in existing libraries. We demonstrate that DynamicLRP produces attributions matching or exceeding specialized
implementations across vision and language tasks while maintaining practical efficiency on models with 100 million to 1 billion parameters. 
Remarkably, our implementation achieved 99.92\% operation coverage across 31,465 computation nodes from 15 diverse architectures, including
transformer models, convolutional networks, state-space models, and multimodal systems, without requiring any architecture-specific modifications. 
This represents the first truly model-agnostic LRP solution, enabling high-quality neural network interpretation across the full spectrum of modern AI
architectures. All code is available at \url{https://github.com/keeinlev/dynamicLRP}.
\end{abstract}
...
processes of large-scale architectures e.g. transformer-based foundation models \cite{gemmateam2025gemma3, vdosovitskiy2021vit, dubey2024llama3} remain challenging to interpret comprehensively.
...
Rule-based decomposition methods, such as Layer-wise Relevance Propagation (LRP) \cite{bach2015pixel}, have demonstrated
their effectiveness through systematic redistribution of the prediction, also known as relevance score. Unlike gradient-based methods, which reflect local sensitivity,
...
few mentioned methods which is not model-agnostic. Notably, Machiraju et al.'s \cite{machiraju2024prospector} comprehensive
...
Our novel \textbf{"Promise System"} addresses the challenge of missing activations during the backward pass by leveraging 
the computation graph used in auto-differentiation. This system preserves the theoretical guarantees of LRP while ensuring 
computational efficiency, adhering to the asymptotic complexity of standard backpropagation.
...
\begin{itemize}
  \item \textbf{Model-agnostic, operation-level LRP framework} (Sec~\ref{sec:op-level-lrp}): We redefine LRP from a module-centric approach to an operation-level paradigm, implementing propagation rules for 47 primitives (See Appendix~\ref{appendix:covered-operations}). This shift ensures model-agnostic applicability, enabling seamless integration across diverse architectures without requiring architecture-specific modifications.
  \item \textbf{Promise System} (Sec~\ref{sec:promise-system}): We propose a novel mechanism for deferred relevance propagation and backward pass activation retrieval, leveraging the computation graph to enable LRP without modifying the graph structure. This system generalizes to arbitrary non-gradient computations within auto-differentiation frameworks. Furthemore we have established and validated complexity bounds for both memory and computational efficiency.
  \item \textbf{Faithfulness validation} (Sec~\ref{sec:experiments}): We demonstrate empirically that DynamicLRP matches or surpasses specialized implementations in vision (ViT-b-16, VGG16) and text (RoBERTa, T5-Flan) tasks, while maintaining practical efficiency in terms of memory and runtime.
\end{itemize}
...
\subsection{Modern LRP Implementations}
LRP has evolved significantly since its inception, with modern implementations striving to achieve greater flexibility and efficiency.
Several libraries, including \texttt{iNNvestigate} \cite{alber2018innvestigate}, \texttt{Captum} \cite{kokhlikyan2020captum}, and Zennit 
\cite{anders2021zennit}, provide implementations of LRP alongside other interpretability methods.
Among these, Zennit stands out for its comprehensive support for rule-based methods in PyTorch, offering a modular framework that leverages
the automatic  differentiation machinery to backpropagate relevances intead of gradients. Another recent advancement in the field that 
is close to our work is \texttt{AttnLRP} \cite{achtibat2024attnlrp}, which derived new propagation rules suitable for attention modules 
and provides an implementation named LxT (LRP eXplains Transformers) built on top of Zennit. Note that at the time of writing, the list of 
architectures currently supported by LxT is limited to Llama 2/3, Gemma 3, Qwen 2/3, BERT, GPT-2, and Vision Transformers. While LxT 
demonstrates the potential of extending LRP to modern architectures, it also illustrates the challenges of adapting module-centric designs 
and  highlights the importance of a more flexible, architecture-agnostic approach.


While the implementation of new rules is claimed to be trivial in Zennit, there is nonetheless significant conceptual overhead in understanding 
how to correctly merge, split, and modify modules to fit within the framework, as well as additional management overhead
due to the need for new configurations and codebase semantics when supporting other architectures. In particular, each model
 requires a custom, duplicated implementation to integrate the appropriate LRP propagation rules throughout the network. 
 We build upon the foundation established by LxT and Zennit, acknowledging their flexibility while addressing the limitations of its 
 module-centric approach. Specifically, adapting Zennit to previously unseen architectural patterns often requires the manual definition
 of new module mappings, a task that our operation-level approach seeks to automate.



\section{Preliminaries}
\subsection{Classic LRP via Taylor Approximation and LRP variants}
\label{methods:taylor}

Classic LRP redistributes the prediction score (relevance) backwards through the layers of a neural network. To design a faithful attribution method, the challenge lies in identifying a meaningful distribution rule $R_{i\leftarrow j}$ (the relevance assigned to input $i$ by output neuron $j$).

In this paper, we take advantage of the Deep Taylor Decomposition framework~\cite{montavon2017explaining, achtibat2024attnlrp} to locally linearize and decompose neural network operations into independent contributions. The framework does this by computing a first-order Taylor expansion of a function $f_j(x)$ at a reference point $x_0$:
\begin{align}
      f_j(x) &= f_j(x_0) + \sum_i \frac{\partial f_j}{\partial x_i}\Big|_{x_0} (x_i - x_{0,i})  + \mathcal{O}(|x -x_0|^2) \nonumber \\
      &= \sum_i \frac{\partial f_j}{\partial x_i}\Big|_{x_0} x_i + \tilde{b}_j
     \label{eq:taylor}
\end{align} 
where $\tilde{b}_j$ collects the constant terms and approximation error.
Assuming the relevance $R_j^l$ is proportional to the function activation $f_j(x)$, we can multiply each term in the expansion by a constant ratio $\frac{R_j^l}{f_j(x)}$ to obtain a recursion for relevance:

\begin{equation*}
     R_j^l = \sum_i \underbrace{\frac{\partial f_j}{\partial x_i}\Big|_{x_0} x_i \frac{R_j^l}{f_j(x)}}_{R_{i\leftarrow j}} + \underbrace{\tilde{b}_j \frac{R_j^l}{f_j(x)}}_{R_{b\leftarrow j}}
\end{equation*}

\textbf{The $\epsilon$-LRP Rule}
Assuming the bias contribution is negligible or handled separately (see \cite{achtibat2024attnlrp} for a detailed discussion on bias absorption), the input relevance can be computed by summing over all output neurons and introducing a stabilizing factor $\epsilon$ to properly handle vanishing denominators. For a linear layer $z_j = \sum_i x_i w_{ji} + b_j$ with $x_0 = 0$:
\begin{equation}
    R_{i \leftarrow j}^{l - 1} = \frac{x_i w_{ji}}{z_j + \epsilon} R_j^l
    \label{eq:assignment}
\end{equation}
This recovers the standard $\epsilon$-LRP rule. In matrix form, efficiently implementable on GPUs:
\begin{equation}
    R^{l-1} = x \odot (W^\top \cdot (R^l \oslash (z + \epsilon)))
\end{equation}
where $\odot$ denotes the Hadamard product and $\oslash$ element-wise division.

\textbf{The $\gamma$-LRP Rule}
Attributions can become noisy when using the $\epsilon$-LRP rule, particularly in scenarios where the denominator $z_j + \epsilon$ becomes dominated by the stabilizing factor $\epsilon$, leading to overly smoothed or less discriminative relevance scores. To address this, the $\gamma$-LRP rule was introduced by Montavon et al. \cite{montavon2019lrp}. 
It adds a hyperparameter $\gamma$ that acts as a multiplicative booster for positive neuron contributions:

\begin{equation}
  R_{i \leftarrow j}^{l - 1} = \frac{x_i \cdot (w_{ji} + \gamma w_{ji}^+)}{\sum_i x_i \cdot (w_{ji} + \gamma w_{ji}^+)} R_j^l
\end{equation}

\subsubsection{Attribution in Transformer Architectures}
The transformer architecture \cite{vaswani2017attention} heavily realies on the non-linear attention mechanism:
\begin{align}
   & A = \text{softmax}\left(\frac{Q \cdot K^\top}{\sqrt{d_k}}\right) \\
   & O = A \cdot V \label{eq:attention}\\
   & \text{softmax}_j(x) = \frac{e^{x_j}}{\sum_k e^{x_k}} 
\end{align}
where ($\cdot$) denotes matrix multiplication, $K, Q, V$ are the key, query and value matrices, $d_k$ is the embedding dimension, and $O$ is the final output.
...
\textbf{Bilinear Attention Matrix Multiplication} The relevance distribution for cell $A_{ji}$ in the matrix multiplication $O = A \cdot V$ is defined as:
\begin{equation}
...
\textbf{Softmax} The relevance at the input layer of the softmax is defined as:
\begin{equation}
...
Most deep learning frameworks rely on computation graphs to represent the sequence of operations performed on tensors and trace the gradients of the loss back to the network parameters.
...
The following proposition formalizes the correctness of this mechanism.

\textbf{Proposition 1:} For any node $v$ in a computation graph, there exists at least one node descendent to $v$
which stores a forward activation.\textit{i.e.,} its Arg Node.

The proof is provided in Appendix~\ref{appendix:proposition-1-proof}.
...
\subsection{Measuring Faithfulness of Visual Attribution}
...
\section{Algorithms}
\begin{algorithm}[H]
  \caption{Computation Graph Pre-Processing}
  \label{alg:graph-construction}
  \begin{algorithmic}[1]
    \Require Model output \texttt{hidden\_states}
    \Ensure In-adjacency list \texttt{in\_adj}, Out-adjacency list \texttt{out\_adj}, Topologically sorted nodes \texttt{topo\_stack}
    \State Initialize $\texttt{in\_adj} \gets \emptyset$, $\texttt{out\_adj} \gets \emptyset$
    \State Initialize $\texttt{visited} \gets \emptyset$, $\texttt{topo\_stack} \gets \emptyset$
    \State $\texttt{root} \gets \texttt{hidden\_states.grad\_fn}$
    \State \Call{DFS}{\texttt{root, in\_adj, out\_adj, visited, topo\_stack}} \Comment{Start traversal}
    \Return $\texttt{in\_adj, out\_adj, topo\_stack}$ \Comment{In-adjacency list, Out-adjacency list, Topologically sorted nodes}
    \Function{DFS}{\texttt{node, in\_adj, out\_adj, visited, topo\_stack}}
      \If{$\texttt{node} = \text{None}$ or $\texttt{node} \in \texttt{visited}$} \Comment{Skip processed}
        \State \Return
      \EndIf
      \State $\texttt{visited} \gets \texttt{visited} \cup \{\texttt{node}\}$ \Comment{Mark visited}
      \For{each $\texttt{child} \in \texttt{node.next\_functions}$}
        \State \texttt{out\_adj[node].append(child)} \Comment{Record dependencies}
        \State \texttt{in\_adj[child].append(node)}
        \State \Call{DFS}{\texttt{child, in\_adj, out\_adj, visited, topo\_stack}} \Comment{Recurse}
      \EndFor
      \State \texttt{topo\_stack.push(node)} \Comment{Add to sort}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require Model output \texttt{hidden\_states}, Relevance attribution target \texttt{target\_node}, Incoming relevance $R_{out}$
    \Ensure Propagated relevance $R_{in}$
    \State Initialize $\texttt{stack} \gets [\texttt{hidden\_states.grad\_fn}]$
    \State Initialize $\texttt{nodes\_pending} \gets \{ \texttt{node} : \text{in-degree of } \texttt{node} \text{ for all nodes in graph}\}$

    \Repeat
      \State $\texttt{curnode} \gets \texttt{stack.pop()}$ \Comment{Process next node}
      \State $\texttt{curnode\_in\_rel} \gets \text{Relevance accumulated at } \texttt{curnode}$
      \If{\texttt{curnode} is \texttt{target\_node}} \Comment{Target reached \Return $R_{in} := \texttt{curnode\_in\_rel}$}
      \ElsIf{\texttt{curnode} requires Promise}
        \State Create new Promise $P$ using \texttt{type(curnode)}
        \State Assign each child of \texttt{curnode} its respective branch of $P$ \Comment{Defer computation}
      \Else
        \State Execute propagation function for \texttt{curnode} \Comment{Applies LRP rule}
        \State Assign each child of \texttt{curnode} its respective split of the relevance
      \EndIf
      \For{each \texttt{child} of \texttt{curnode}}
        \State Decrement \texttt{nodes\_pending[child]} by 1 \Comment{Update dependencies}
        \State Push \texttt{child} to \texttt{stack} if \texttt{nodes\_pending[child]} is 0 \Comment{Enqueue ready children}
      \EndFor
    \Until{\texttt{stack} is empty} \Comment{All nodes processed}

  \State \Return Input relevance $R_in$
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require Autograd Node \texttt{node}, Propagation input $R_{out}$
    \Ensure Propagation output list $R_{in}$

    \If{$R_{out}$ is a Promise Branch} \Comment{Record chain if exploring}
      \State Define \texttt{fwd} as a closure of \texttt{node}'s forward pass.
      \State Define \texttt{bwd} as a closure of \texttt{node}'s relevance distribution logic.
      \State $R_{out}\texttt{.record(fwd, bwd)}$
      \Return $R_{out}$
    \EndIf

    \State \textit{// Propagate $R_{out}$...} \Comment{Standard LRP}
    \State \Return $R_{in}$
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \If{$R_{out}$ is a Promise Branch} \Comment{Retrieve forward activation}
      \State Define \texttt{retrieve\_fwd\_output} as a function that extracts \texttt{node}'s forward pass output given its saved tensors.
      \State \texttt{activation = retrieve\_fwd\_output(node)}
      \State $R_{out}\texttt{.set\_arg(activation)}$
      \State $R_{out}\texttt{.trigger\_promise\_completion()}$
      \If{$R_{out}\texttt{.promise\_is\_complete}$} \Comment{Resume propagation}
        $R_{out} = R_{out}\texttt{.propagated\_relevance}$
      \Else
