...
We motivate shifting the level of abstraction from layer-wise to operation-wise by careful consideration of the modern-day deep learning paradigm
and the costs of the practices used by currently existing LRP implementations.

When operating at the layer level, for every new model architecture, a configuration for LRP must be specified for each new module
in the architecture. Since modules are user-defined, there is thus no limit to the number of such unique configurations. And though many
architectures may share the same set of tensor operations used in their computations, they would require two different configurations in this case.
By using this level of abstraction, current LRP implementations also require many modules in the model to be patched and hooked, changing
built-in behaviours of forward and backward passes. This introduces a potential entrypoint for user error, should they use the wrong version of the
model, or forget to switch off these patches for true training or evaluation. We aim to curb these concerns by enabling LRP at the operation
level.
Another oversight of using layer-wise abstraction for LRP is that there are many operations that can be done to tensors which do not
materialize as modules (e.g. addition, multiplication, logarithms, etc.), and thus must be written in explicitly as sub-steps in a wider-scoped,
and module-specific propagation rule any time they are to be considered in LRP.

By atomicizing relevance propagation to the core operations applied to tensors, we eliminate the need for redundant configurations,
maximize reuse of defined mathematical rules, and ensure the capture of any and all significant manipulations to the data made by the model
without repeating propagation rule definitions, regardless of the module they are defined in.
Additionally, our framework sits on top of the auto-differentiation module, not within it, so there is no interference with the built-in
systems which may compromise regular functionality.
This operation-level approach modularizes relevance propagation, reducing any model architecture to a fixed set of fundamental
tensor operations.
...
\begin{enumerate}
  \item \textbf{Forward Pass:} Get the model output and computation graph built by the auto-differentiation module.
