...
\iftoggle{arxiv}{
  ...
  anode/.style={circle, draw=orange!60, fill=orange!5, very thick, minimum size=7mm},
  bnode/.style={circle, draw=red!60, fill=red!5, very thick, minimum size=7mm},
  cnode/.style={circle, draw=blue!60, fill=blue!5, very thick, minimum size=7mm},
  dnode/.style={circle, draw=teal!60, fill=teal!5, very thick, minimum size=7mm},
}
...
Input Aggregation
When a node has multiple in-neighbours pointing to one input position, we must aggregate the incoming relevances into one object.
For multiple tensor inputs, this is straightforward, simply sum them.
For multiple Promise objects, we need to group them somehow while maintaining correct structure for resolution.


Proposition 1: The computation graph traversal does not backtrack and retraverse any node once it has already propagated relevance through it.
Proof:
If the node is given tensor values to propagate, and it is either not a promise-generating operation or a dependent promise-generating operation, then it will return a tensor relevance.
The value will be propagated to the node's outneighbours, which will then be possibly added to the traversal order if all their inputs have landed.
Since the computation graph is by definition a DAG (as no instance of an operation can create a dependency for itself), there is no manner in which the traversal will come back to the starting node.

If the node is given tensor values to propagate, and it is a strict promise-generating operation, the same logic applies, but we pass along a Promise object instead of a tensor relevance.
The Promise resolution algorithm ensures that the traversal pointer will never backtrack, as it performs all necessary forward and backward propagations in-place at the frontier.
Therefore, the graph traversal will never come back to the starting node.

If the node is given a Promise to propagate, and it is a promise-generating operation, there are 2 cases to consider:
1. If this propagation is occuring as part of the regular traversal heuristic, the same logic of the previous cases applies.
2. If this propagation has broken the traversal heuristic, and the newly-created Promise is a Pre-Promise, there is additional reasoning to be done.

In case 2, consider that the Pre-Promise resolution will always take priority over the traversal heuristic.
...
Claim: The internal nodes of a given Promise chain are unique to that Promise.
Proof:
By Prop. 1 we have that no node has relevance propagated through it more than once in our graph traversal.
This includes propagation via Promise chains.
...
%Nodes
\node[bnode] (node1) {A};
\node[bnode, label={\small stalled}] (node2) [right=of node1] {B};
\node[node] (node3) [right=of node2] {C};
\node[node] (node4) [below=of node3, xshift=-8mm] {D};
\node[anode] (node5) [right=of node3] {E};
...
\node[bnode, label={\small stalled}] (node2) [right=of node1] {B};
\node[node] (node3) [right=of node2] {C};
\node[bnode] (node4) [below=of node3, xshift=-8mm] {D};
\node[anode] (node5) [right=of node3] {E};
...
See Appendix for the formal safety proof that Pre-Promises maintain correctness.
...
\node[node] (node3) [right=of node2] {C};
\node[bnode] (node4) [below=of node3, xshift=-8mm] {D};
\node[bnode] (node5) [right=of node3] {E};
...
%Nodes
\node[dnode] (node1) {A};
\node[dnode, label={\small stalled}] (node2) [right=of node1] {B};
\node[node] (node3) [right=of node2] {C};
\node[dnode] (node4) [below=of node3, xshift=-8mm] {D};
\node[dnode] (node5) [right=of node3] {E};
...
%Nodes
\node[cnode] (node1) {A};
\node[cnode] (node2) [right=of node1] {B};
\node[node] (node3) [right=of node2] {C};
\node[cnode] (node4) [below=of node3, xshift=-8mm] {D};
\node[dnode] (node5) [right=of node3] {E};
...
\node[cnode] (node1) {A};
\node[cnode] (node2) [right=of node1] {B};
\node[bnode] (node3) [right=of node2] {C};
\node[cnode] (node4) [below=of node3, xshift=-8mm] {D};
\node[dnode] (node5) [right=of node3] {E};
...
\node[cnode] (node1) {A};
\node[cnode] (node2) [right=of node1] {B};
\node[cnode] (node3) [right=of node2] {C};
\node[cnode] (node4) [below=of node3, xshift=-8mm] {D};
\node[bnode] (node5) [right=of node3] {E};
...
\textbf{Promise-Aware LRP Algorithm}
\begin{algorithm}[H]
